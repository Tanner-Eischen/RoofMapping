...
- [x] Queue client module created (`src/queues/sqs.ts`)
- [x] sendMessage method implemented (enqueue stub)
- [ ] Message format defined: {analysisId, address, coordinates}
- [x] Error handling for queue failures (graceful catch)
- [ ] Fallback: update analysis status to "failed" if queue submission fails
- [x] Queue URL configured via environment variable (SQS_QUEUE_URL)
