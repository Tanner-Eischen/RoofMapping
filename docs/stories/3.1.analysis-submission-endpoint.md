...
## Acceptance Criteria
- [x] API route created (`app/api/analyze/route.ts`)
- [ ] Request validation with Zod schema (address, lat, lng)
- [x] Create analysis record (queued/processing)
- [x] Return 202 Accepted with analysisId
- [x] Error handling for invalid requests (400)
- [x] Error handling for database failures (500)
- [ ] Logging of analysis submissions
- [ ] Rate limiting configured (100 requests/hour per IP)

## Tasks / Subtasks
- [x] Implement `POST /api/analyze` route (AC: 1)
- [ ] Add Zod schema validation (AC: 2)
- [x] Create DB record with status queued/processing (AC: 3)
- [x] Return 202 with `analysisId` (AC: 4)
- [x] Add error handling paths (AC: 5-6)
